...
<head>
    ...
    <title>Tambah Anggota Silsilah | Admin Panel</title>
    ...
    <main class="flex-1 flex flex-col overflow-y-auto">
        ...
        <div class="p-8 max-w-4xl mx-auto w-full space-y-8">
            ...
            <div class="flex items-center gap-2 text-xs text-brand-light/60">
                ...
                <span class="text-white font-medium">Tambah Anggota</span>
            </div>

            <!-- Title -->
            <div>
                <h2 class="font-display font-extrabold text-2xl text-white">Tambah Anggota Keluarga</h2>
                <p class="text-brand-light/70 text-xs mt-1">Masukkan informasi data diri dan silsilah anggota keluarga baru.</p>
            </div>
            ...
            <div class="bg-gradient-to-b from-brand-dark/20 to-brand-dark/5 border border-brand-medium/20 rounded-2xl p-8 shadow-lg">
                ...
                <?= form_open_multipart('admin/silsilah_add', ['class' => 'space-y-6']) ?>
                    ...
                    <div class="border-b border-[#4D6B67]/20 pb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="full_name" required value="<?= set_value('full_name') ?>" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="user_id" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                                    ...
                                    <?php foreach ($unlinked_users as $usr): ?>
                                        <option value="<?= $usr['id'] ?>" <?= set_select('user_id', $usr['id']) ?>><?= htmlspecialchars($usr['full_name']) ?> (<?= htmlspecialchars($usr['email']) ?>)</option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-white/40 mt-1.5">Format file: JPG, PNG, WEBP. Maks: 2MB.</p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div>
                                ...
                                <select name="father_id" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                                    ...
                                    <?php foreach ($fathers as $fat): ?>
                                        <option value="<?= $fat['id'] ?>" <?= set_select('father_id', $fat['id']) ?>><?= htmlspecialchars($fat['full_name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="mother_id" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                                    ...
                                    <?php foreach ($mothers as $mot): ?>
                                        <option value="<?= $mot['id'] ?>" <?= set_select('mother_id', $mot['id']) ?>><?= htmlspecialchars($mot['full_name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="border-b border-[#4D6B67]/20 pb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="birth_place" value="<?= set_value('birth_place') ?>" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                            </div>

                            <!-- Tanggal Lahir -->
                            <div>
                                <label class="block text-xs font-semibold text-white/70 mb-2 uppercase tracking-wide">Tanggal Lahir</label>
                                <input type="date" name="birth_date" value="<?= set_value('birth_date') ?>" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="is_alive" id="is_alive" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                                    <option value="1" <?= set_select('is_alive', '1', TRUE) ?>>Masih Hidup</option>
                                    <option value="0" <?= set_select('is_alive', '0') ?>>Sudah Wafat</option>
                                </select>
                            </div>

                            <!-- Tanggal Wafat (Hidden by default) -->
                            <div id="death_date_container" class="hidden md:col-span-3">
                                ...
                                <input type="date" name="death_date" value="<?= set_value('death_date') ?>" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pb-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="phone" value="<?= set_value('phone') ?>" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                            </div>

                            <!-- Email -->
                            <div>
                                <label class="block text-xs font-semibold text-white/70 mb-2 uppercase tracking-wide">Email</label>
                                <input type="email" name="email" value="<?= set_value('email') ?>" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                            </div>

                            <!-- Pekerjaan -->
                            <div>
                                <label class="block text-xs font-semibold text-white/70 mb-2 uppercase tracking-wide">Pekerjaan</label>
                                <input type="text" name="occupation" value="<?= set_value('occupation') ?>" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all">
                            </div>

                            <!-- Alamat Lengkap -->
                            <div class="md:col-span-3">
                                <label class="block text-xs font-semibold text-white/70 mb-2 uppercase tracking-wide">Alamat Lengkap</label>
                                <textarea name="address" rows="3" class="w-full bg-[#1A2824] border border-[#4D6B67]/30 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-brand-medium transition-all"><?= set_value('address') ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-6 border-t border-[#4D6B67]/20">
                        ...
                        <button type="submit" class="bg-brand-medium hover:bg-brand-medium/90 border border-brand-medium text-white px-6 py-3 rounded-xl text-sm font-bold shadow-md transition-all">Simpan Anggota</button>
                    </div>
